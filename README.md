...
- 桌面宽屏下，鼠标靠近右上角显示控制面板，离开后隐藏。
- 窄屏或竖屏下，控制面板在底部，鼠标靠近底部显示。
- 触控设备上控制面板保持可见。
...
- 因此检测框会跟随视频缩放和裁切
- 上传视频或 JSON 后会从头开始播放，避免后补 JSON 时不同步
- 视频会循环播放，循环回到开头时检测框同步回到第一帧
- 新 JSON 中的 `color` 字段会用于前端框线和标签配色；旧 JSON 没有 `color` 时会按 `track_id` 自动生成颜色
